...
class BehavioralSessionGenerator:
    ...
    def load_real_orders(self):
        ...
        item_count = (
            order_items.groupby("order_id").size().reset_index(name="item_count")
        )
    ...
    def generate_all(self):
        ...
        session_duration = np.clip(
            pages_viewed * T_page + np.random.normal(0, 40, size=n), 10, 3600
        ).astype(int)

        cart_additions = self.orders["item_count"].values + np.random.randint(
            0, 4, size=n
        )
        ...
        discount[mask] = np.random.choice(
            [5, 10, 15, 20], size=mask.sum(), p=[0.40, 0.35, 0.15, 0.10]
        )
