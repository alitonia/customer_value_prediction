...
MAJOR_CITIES = {
    ...
    "goiania", "guarulhos", "campinas", "sao luis", "sao goncalo",
    "maceio", "duque de caxias", "nova iguacu", "teresina",
...
def clean(raw: dict[str, pd.DataFrame]) -> tuple[pd.DataFrame, dict]:
    ...
    def record(step, before, after, detail=""):
        stats["steps"].append({"step": step, "before": before, "after": after, "detail": detail})
        log.info("  [%s] %d → %d %s", step, before, after, f"({detail})" if detail else "")
    ...
    record("3.3a Filter to delivered/shipped", before, len(orders), f"removed {before - len(orders)}")
    ...
    # Customer geo
    cust_geo = customers[["customer_id", "customer_unique_id", "customer_zip_code_prefix", "customer_state"]].merge(
        geo_agg, left_on="customer_zip_code_prefix", right_on="geolocation_zip_code_prefix", how="left"
    )
    cust_geo["customer_urban"] = cust_geo["geo_city"].str.lower().isin(MAJOR_CITIES).astype(int)
    ...
    items_with_cat = items.merge(products[["product_id", "product_category_name_english"]], on="product_id", how="left")
    seller_cat = items_with_cat.groupby("seller_id")["product_category_name_english"].nunique().reset_index()
    seller_cat.columns = ["seller_id", "seller_n_categories"]
    seller_agg = seller_agg.merge(seller_cat, on="seller_id", how="left")
    seller_agg = seller_agg.merge(seller_geo[["seller_id", "seller_state", "geo_lat", "geo_lng"]],
                                   on="seller_id", how="left", suffixes=("", "_seller"))
    seller_agg.rename(columns={"geo_lat": "seller_lat", "geo_lng": "seller_lng"}, inplace=True)

    # Primary seller per order (first item's seller)
    order_seller = items.sort_values("order_item_id").drop_duplicates("order_id")[["order_id", "seller_id"]]

    # --- Product category aggregates ---
    log.info("  Computing product-level aggregates...")
    cat_price = items.merge(products[["product_id", "product_category_name_english"]], on="product_id", how="left")
    cat_agg = cat_price.groupby("product_category_name_english").agg(
    ...
    order_cat = items_with_cat.groupby("order_id").agg(
    ...
    act_agg = activity.groupby("session_id").agg(
    ...
    cust_first = orders_sorted.groupby("customer_id")["order_purchase_timestamp"].min().rename("cust_first_order")
    cust_prev = orders_sorted.groupby("customer_id")["order_purchase_timestamp"].apply(
        lambda x: x.shift(1)
    ...
    rfm["customer_age_days"] = (rfm["order_purchase_timestamp"] - rfm["cust_first_order"]).dt.days
    rfm["recency_days"] = (rfm["order_purchase_timestamp"] - rfm["cust_prev_order"]).dt.days
    ...
    df = df.merge(customers[["customer_id", "customer_unique_id", "customer_state", "customer_zip_code_prefix"]], on="customer_id")
    df = df.merge(cust_geo[["customer_id", "geo_lat", "geo_lng", "customer_urban"]], on="customer_id", how="left")
    df = df.merge(order_cat, on="order_id", how="left")
    df = df.merge(cat_agg, left_on="primary_category", right_on="product_category_name_english", how="left")
    df = df.merge(order_seller, on="order_id", how="left")
    df = df.merge(seller_agg, on="seller_id", how="left")
    df = df.merge(profile, left_on="customer_unique_id", right_on="customer_id", suffixes=("", "_profile"))
    df = df.merge(sessions.drop(columns=["customer_id"], errors="ignore"), on="order_id", suffixes=("", "_session"))
    df = df.merge(act_agg, on="session_id", how="left")
    df = df.merge(rfm[["order_id", "customer_age_days", "recency_days", "frequency",
                        "purchase_month", "purchase_quarter", "purchase_day_of_month"]], on="order_id")

    # Payment info
    pay_agg = raw["payments"].groupby("order_id").agg(
    ...
    # Review score
    review_agg = raw["reviews"].groupby("order_id").agg(review_score=("review_score", "mean")).reset_index()
    ...
    df["cust_seller_distance_km"] = _haversine(
        df["geo_lat"].fillna(-15.78), df["geo_lng"].fillna(-47.93),
        df["seller_lat"].fillna(-15.78), df["seller_lng"].fillna(-47.93),
    )

    # Drop post-purchase / identifier columns
    drop_cols = [
        "order_approved_at", "order_delivered_carrier_date", "order_delivered_customer_date",
        "order_estimated_delivery_date", "shipping_limit_date", "customer_zip_code_prefix",
        "geolocation_zip_code_prefix", "geo_city", "geo_state",
        "cust_first_order", "cust_prev_order",
    ]
    ...
    for c in list(df.columns):
        if pd.api.types.is_datetime64_any_dtype(df[c]) and c not in (
            "order_purchase_timestamp", "session_start", "session_end", "birth_date",
        ):
    ...
    if len(cols_with_nulls) > 0:
        ...
        for col in cols_with_nulls.index:
            if pd.api.types.is_numeric_dtype(df[col]):
                ...
                df[col] = df[col].fillna(df[col].mode().iloc[0] if len(df[col].mode()) > 0 else pd.NaT)
            else:
...
def write_cleaning_log(stats: dict, output_dir: Path) -> None:
    ...
    for s in stats["steps"]:
        lines.append(f"| {s['step']} | {s['before']:,} | {s['after']:,} | {s['detail']} |")
...
def main():
    ...
    df.to_parquet(args.output_dir / "modeling_dataset.parquet", index=False, compression="snappy")
    ...
    (args.output_dir / "cleaning_stats.json").write_text(json.dumps(stats, indent=2, default=str))
    log.info("Done.")
